...
Thanks for helping improve RawKit. Bug reports, focused fixes, documentation improvements, and well-scoped features are welcome.
...
- Search [existing issues](https://github.com/focale-editor/rawkit/issues) before opening a new one.
...
RawKit follows the SDK constraint in [`pubspec.yaml`](pubspec.yaml). Install a compatible stable Dart SDK, fork the repository, and run:

```sh
git clone https://github.com/YOUR_ACCOUNT/rawkit.git
cd rawkit
dart pub get
```
...
Native builds, optional corpus tests, and platform prerequisites are documented in [docs/testing.md](docs/testing.md).
